...
(function(window, document, Math) {
  ...
  function IScroll(el, options) {
    ...
    this.options = {
      probeType: 3, //默认为undefine,可设置为1，2，3。主要用于开启监听scroll，值越大灵敏度越高，越耗性能。
      click: false, //自定义事件，配置为 string,需要执行的自定义事件
      tap: false, //自定义事件，配置为 string,需要执行的自定义事件
      mouseWheelSpeed: 20, //鼠标滚轮速度
      snap: false, //设置为 true 后，可按一定步长移动
      snapThreshold: 0.334, //默认为0.334，移动到下一步长的阀值系数，此值乘对应页的宽或高
      infiniteUseTransform: true,
      deceleration: 0.0006, //滚动动量减速，越大越快，建议不大于0.01
      ...
      startY: 0,
      scrollY: true, //开启纵向滚动
      directionLockThreshold: 1, //默认值为5，锁定方向的一个阀值，一个方向持续滑动超过这个阀值之后，才会锁定另一方向，一般不需要改动。
      momentum: true, //默认为 true，运动的物理引擎，对性能有影响，可关闭
      bounce: true, //是否启用弹力动画效果，关掉可以加速
      bounceTime: 500, //回弹时间
      bounceEasing: '', //默认为 circular，弹动效果
      preventDefault: false, //阻止默认事件
      preventDefaultException: { tagName: /^(INPUT|TEXTAREA|BUTTON|SELECT)$/ }, //和preventDefault，即内部函数isBadAndroid配合使用，在各个运动函数中执行
      HWCompositing: true, //是否采用 3D 加速，同时 iscroll 内部还会对客户端是否支持 perspective 属性共同决定
      useTransition: true, //是否使用CSS3的Transition属性，否则使用requestAnimationFram代替
      useTransform: true, //是否使用CSS3的Transform属性
      bindToWrapper: typeof window.onmousedown === 'undefined', //默认为undefine，控制初始化事件绑定到什么对象上。为true时，会将touch,mousemove等事件绑定到iscroll的dom对象。否则绑定到window上
    };
    ...
    this.options.freeScroll = this.options.freeScroll && !this.options.eventPassthrough; //默认为 flase，scrollX 和此值同时设置为 true 时，可想任意方向滚动
    ...
    this.options.resizePolling =
      this.options.resizePolling === undefined ? 60 : this.options.resizePolling; //默认值 60ms，控制下拉刷新时间
    ...
    if (this.options.infiniteElements) {
      this.options.probeType = 3; //默认为 undefine, 可设置为1，2，3。主要用于开启监听 scroll， 值越大灵敏度越高，越耗性能
    }
  ...
  IScroll.prototype = {
    ...
    }, // destory只会销毁所有绑定事件，并不会销毁 iscroll。可通过 iscroll = null 对 iscroll进行销毁。此函数还会触发 destory 事件
    ...
    _end: function(e) {
      ...
      // start momentum animation if needed //计算惯性力度和时间
    ...
    refresh: function() {
      ...
      if (this.options.infiniteElements) {
        this.options.infiniteLimit =
          this.options.infiniteLimit || Math.floor(2147483645 / this.infiniteElementHeight);
    ...
    }, //调整容器位置，［注意］每次新增或删除 dom 节点后需调用此方法
    ...
    scrollTop: function() {
      ...
      var that = this;
      rAF(function() {
        that.scrollTo(0, 0, 300);
      });
    }, //lewis 终止前一滚动过程，直接回顶部
    ...
    reorderInfinite: function() {
      ...
      var minorPhase = Math.max(
          Math.floor(-this.y / this.infiniteElementHeight) - this.infiniteUpperBufferSize,
          0,
        ),
